    ...
    def input(self):
        ...
        if self.can_move:
            if keys[pygame.K_RIGHT]:
                ...
                self.can_move = False
                self.selection_index = self.selection_index % self.attribute_nr
...
class Item:
    ...
    def display_names(self, surface, name, cost, selected):
        ...
        # cost
        cost_surf = self.font.render(f'cost : {str(cost)}', False, color)
        cost_rect = title_surf.get_rect(
            midbottom=self.rect.midbottom - pygame.math.Vector2(37, 20))
    ...
    def trigger(self, player):
        ...
        if player.exp >= player.upgrade_cost[upgrade_attr] and player.stats[upgrade_attr]<player.max_stats[upgrade_attr]:
            ...
            player.upgrade_cost[upgrade_attr]=int(player.upgrade_cost[upgrade_attr]*1.4)
